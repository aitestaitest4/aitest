...
const upload = multer({ storage }).single("file"); // Expect single file with field name 'file'
...
const uploadAudio = (req, res, next) => {
  upload(req, res, (err) => {
    ...
    if (!req.file || !req.file.buffer) {
      return res
        .status(400)
        .json({ error: "No audio file uploaded or file is invalid" });
    }

    // File is available, proceed with file processing
    req.fileLink = req.file; // You can pass the file information here
    next();
  });
};

const uploadImageBackground = async (req, res, next) => {
  try {
    const file = req.file;
...
const magicstudioAiUploadImageBackground = async (req, res, next) => {
  try {
    const file = req.file_image;
    // const mask = req.file_mask;
    const fileLink = await uploadToFileIO(file);
    // const maskLink = await uploadToFileIO(mask);
    req.fileLink = fileLink;
    // req.maskLink = maskLink;
    next();
  } catch (error) {
    console.error("File upload error:", error);
    return res.status(500).json({ error: "File upload failed" });
...
//     // Make corrections based on matches
//     // Sort matches by the index to avoid replacing parts of text incorrectly
//     matches
...
//         // Create a new corrected text by replacing the incorrect match
//         correctedText =
...
//     // Log the corrected text to the console
...
async function correctGrammar(text) {
  try {
    ...
    console.error("Error in grammar and spelling correction:", error);
    return text;
  }
}
...
module.exports = {
  ...
  magicstudioAiUploadImageBackground,
  uploadAudio,
};
